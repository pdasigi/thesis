...
output. In this thesis, we identify two main weaknesses with this generic NLU pipeline, and propose to address them by incorporating
...
knowledge bases like WordNet, ConceptNet and Freebase. 
...
to some extent. However, the memory network models published so far have been shown to be capable of only shallow reasoning. We propose to apply
these models to two tasks that require deeper reasoning. The first is answering non-factoid questions from science text books, and the second is identifying entailment
within experiment narratives in research papers. Our initial results using memory networks on these tasks show that there is a lot of room
...
them overfit less to the task they are trained on and make them more transferable, and whether knowledge augmented encoders learn representations
that are more general, and thus more transferable to other NLU tasks.

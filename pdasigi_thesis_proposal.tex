...
\begin{abstract}
Natural Language Understanding (NLU) systems need to encode, and reason over human generated text (or speech) at a deep semantic level.
...
which are then used as features in the second component, a \textbf{predictor}, to produce the desired output. In this thesis, we argue that the utterances themselves,
...
\textbf{background knowledge} is that which is not explicitly stated in the utterances for the sake of brevity, and \textbf{contextual knowledge}
is that which is only provided by the context in which the utterance occurs. The goal of this thesis is to build end-to-end NLU systems that encode inputs along with relevant
